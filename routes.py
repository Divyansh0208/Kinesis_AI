...
from models import User, Workout, UserStats, InjuryRiskLog
import gemini
...
def register_routes(flask_app):
    ...
    def _resolve_or_create_user(decoded_token, fitness_level=None, fitness_goals=None):
        """Find a local User by firebase_uid, or create one from the token's
        email/uid. Auto-generates a unique username since Firebase (esp.
        Google sign-in) doesn't collect one."""
        uid = decoded_token['uid']
        email = decoded_token.get('email') or f"{uid}@no-email.firebase"

        user = User.query.filter_by(firebase_uid=uid).first()
        if user:
            return user

        # Link an existing account created before Firebase migration, if email matches.
        ...
        base_username = (decoded_token.get('name') or email.split('@')[0])
    ...
    def login():
        if request.method == 'POST':
            ...
            user = _resolve_or_create_user(decoded)
            login_user(user)
            next_page = request.args.get('next')
            return jsonify({'redirect': next_page or url_for('dashboard')})
    ...
    def complete_session():
        """
        Call this once, when a workout set/session ends (frontend has been
        accumulating per-rep form scores from the live pose-detection loop).
        ...
        Expects JSON:
        {
            ...
            "duration_minutes": 6,
            "calories_burned": 45,
            "rep_form_scores": [88, 85, 90, 82, 76, 70, 65],  # one per rep, in order
            "notes": "optional"
        }

        Saves the Workout, runs injury-risk assessment (session drift +
        cross-session trend), awards XP/streak/badges, and returns
        everything the frontend needs to show in one response.
        """
        try:
            data = request.get_json(silent=True) or {}

            exercise_type = data.get('exercise_type', 'general')
            reps_completed = int(data.get('reps_completed', 0) or 0)
            duration_minutes = int(data.get('duration_minutes', 0) or 0)
            calories_burned = int(data.get('calories_burned', 0) or 0)
            rep_form_scores = data.get('rep_form_scores') or []
            notes = data.get('notes', '')

            avg_form_score = (
                sum(rep_form_scores) / len(rep_form_scores) if rep_form_scores else None
            )

            # --- 1. Save the workout ---
            workout = Workout(
                user_id=current_user.id,
                name=data.get('name', exercise_type.title() if exercise_type else 'Workout'),
            ...
            session_risk = injury_risk_engine.assess_session(exercise_type, rep_form_scores)
            ...
            ).order_by(Workout.completed_at.asc()).all()

            trend_risk = injury_risk_engine.assess_trend(
                [{"form_score": w.form_score, "completed_at": w.completed_at} for w in recent_workouts],
                exercise_type,
            )

            combined_risk = injury_risk_engine.combined_risk(session_risk, trend_risk)

            if combined_risk['risk_level'] != 'low':
                risk_log = InjuryRiskLog(
                    user_id=current_user.id,
                    exercise_type=exercise_type,
                    risk_level=combined_risk['risk_level'],
                    risk_score=combined_risk['risk_score'],
                    note=combined_risk['note'],
                    drill=combined_risk['drill'],
                )
            ...
            game_result = gamification_engine.process_workout(
                stats_row=stats,
                reps_completed=reps_completed,
                form_score=avg_form_score,
                duration_minutes=duration_minutes,
            ...
            stats.xp = game_result['total_xp']
            stats.level = game_result['level']
            stats.current_streak = game_result['current_streak']
            stats.longest_streak = game_result['longest_streak']
            stats.last_workout_date = game_result['last_workout_date']
            stats.badges = json.dumps(game_result['badges'])
            db.session.commit()

            return jsonify({
                'success': True,
                'workout_id': workout.id,
                'injury_risk': combined_risk,
                'gamification': {
                    'xp_earned': game_result['xp_earned'],
                    'total_xp': game_result['total_xp'],
                    'level': game_result['level'],
                    'leveled_up': game_result['leveled_up'],
                    'xp_to_next_level': game_result['xp_to_next_level'],
                    'current_streak': game_result['current_streak'],
                    'longest_streak': game_result['longest_streak'],
                    'new_badges': game_result['new_badges'],
                },
            })
    ...
    def leaderboard():
        try:
            ...
            return jsonify({
                'success': True,
                'leaderboard': gamification_engine.leaderboard(all_stats, limit=20),
            })
        except Exception as e:
            logging.error(f"Error building leaderboard: {e}")
            return jsonify({'success': False, 'error': str(e)}), 500
        ...
        try:
            ...
            for workout_data in data.get('workouts', []):
                workout = Workout(
                    ...
                    duration_minutes=workout_data.get('duration', 0),
    ...
    def version():
        return jsonify({'version': '1.0.0', 'name': 'KinesisAI'})
