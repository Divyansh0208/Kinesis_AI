...
DEFAULT_MODEL = os.environ.get("GEMINI_MODEL", "models/gemini-3.6-flash")
...
def _ollama_complete(prompt: str) -> Optional[str]:
    ...
    Has a real timeout and strips <think> blocks from reasoning models
    (e.g. deepseek-r1) so raw chain-of-thought never leaks into a plan.
    """
    model = _ensure_ollama_model()
    if not model:
        return None

    is_reasoning_model = any(tag in model.lower() for tag in ["deepseek-r1", "r1", "qwq"])
    num_predict = 3000 if is_reasoning_model else 900
    timeout_s = 120 if is_reasoning_model else 45
    ...
    try:
        ...
        logging.error(f"Ollama timed out in Gemini helpers (model={model}, timeout={timeout_s}s)")
        return None
    except Exception as e:
