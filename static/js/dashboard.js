...
class DashboardManager {
    ...
    async init() {
        try {
            ...
            Kinesis.utils.showToast('Failed to load dashboard data', 'danger');
        }
    }

    async loadDashboardData() {
        try {
            const response = await Kinesis.api.get('/dashboard-stats');
    ...
    async loadGoogleFitData() {
        try {
            const response = await Kinesis.api.get('/google-fit-data');
    ...
    async sendChatMessage() {
        ...
        try {
            ...
            // Send to AI
            const response = await Kinesis.api.post('/fitness-chat', { question: message });
    ...
    setupEventListeners() {
        ...
        window.addEventListener('kinesis:resize', () => {
    ...
    async updateLiveData() {
        try {
            ...
            const workoutTimer = Kinesis.components.timer.getElapsed('workout');
            if (workoutTimer > 0) {
                const duration = Kinesis.components.timer.formatTime(workoutTimer);
    ...
    async refreshDashboard() {
        ...
        try {
            await this.loadDashboardData();
            this.updateCharts();
            Kinesis.utils.showToast('Dashboard refreshed', 'success');
        } catch (error) {
            Kinesis.utils.showToast('Failed to refresh dashboard', 'danger');
        } finally {
            if (refreshBtn) {
    ...
    exportData() {
        try {
            ...
            link.download = `kinesis-data-${new Date().toISOString().split('T')[0]}.json`;
            link.click();

            Kinesis.utils.showToast('Data exported successfully', 'success');
        } catch (error) {
            console.error('Export error:', error);
            Kinesis.utils.showToast('Failed to export data', 'danger');
        }
    }
